...
void set_speed_car(const std_msgs::Float32::ConstPtr& msg){
    ...
    ROS_DEBUG("Hello debug subscriber");
    ROS_INFO("Hello info subscriber");
    ROS_ERROR("Hello error subscriber");
    if (speed >= 0)
    {
        ROS_DEBUG_STREAM("Hello " << speed);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub1 = n.subscribe("/set_speed_car_api", 50, set_speed_car);
    ros::Subscriber sub2 = m.subscribe("/set_steer_car_api", 50, set_steer_car);
    ros::Subscriber sub3 = l.subscribe("/set_pos_camera_api", 50, set_pos_camera);			
    ros::spin();
    return 0;
}
